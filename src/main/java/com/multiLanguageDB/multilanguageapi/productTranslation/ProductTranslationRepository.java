...
public interface ProductTranslationRepository extends JpaRepository<ProductTranslation, UUID> {
    ...
    @Query("SELECT p FROM ProductTranslation p WHERE p.product.id = :id")
    List<ProductTranslation> findAllByProduct(@Param("id") UUID id);

    @Query("SELECT p FROM ProductTranslation p WHERE p.locale = :locale")
    List<ProductTranslation> findAllByLocale(@Param("locale") String locale);
